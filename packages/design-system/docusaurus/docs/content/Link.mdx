...
- **Border-bottom**: Animated underline on hover (`default` and `subtle` variants)
...
| `variant`   | `'default' \| 'subtle' \| 'plain'`                                              | `'default'`                                         | Visual style variant                                |
| `color`     | `'primary' \| 'secondary' \| 'tertiary'` <br/> `\| 'success' \| 'error' \| 'warning' \| 'info' \| 'inverse'` | `'primary'`; `'secondary'` for `subtle` variant | Text color (semantic tokens) |
...
Same animated hover behavior as `default`, but defaults to secondary text color. Use for de-emphasized links that shouldn't draw strong visual attention.
...
- Use `variant="subtle"` for de-emphasized navigation
